...
- الألوان الأساسية (Brand B — عسل تايم):
  - `--acc:#0d9488` (تركوازي) / `--acc2:#14b8a6` (نعناعي)
  - النص `--ink:#1c1a17` / الثانوي `--mut:#5c554b` / الخلفية `--bg:#fdfbf6` / السطحي `--soft:#ffffff`.
  - Brand A (العراق): `--acc:#a16207` / `--acc2:#d97706` (رملي دافئ) — متغيّر يُولَّد تلقائياً عبر نصوص موحّدة.
